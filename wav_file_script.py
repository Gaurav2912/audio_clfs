...
def whisper_help_wav(wave_file, result_queue, chunk_start, chunk_end, model_wh, max_duration=1.4):
    ...
    with ThreadPoolExecutor(max_workers=4) as executor:
        ...
        try:
            ...
            result_queue.put(("BACKGROUND NOISE", 0.0))
            # raise TimeoutError("Whisper processing exceeded time limit")
   


def function_shout_wav(wave_file, result_queue, chunk_start, chunk_end):
    ...
    chunk = audio[:, chunk_start:chunk_end]
    image_path = transform_data_to_image(chunk, sample_rate)
...
def run_yamnet_wav(filepath, result_queue, chunk_start, chunk_end):
    ...
    waveform = waveform[chunk_start:chunk_end]
    
    # Ensure the chunk is exactly 32000 samples
    if len(waveform) < 16000:
        padding = 16000 - len(waveform)
